#!/usr/bin/env python3
"""3D parameter sweep: peak infections vs (mc, drug_contact_restore, mr, phi_transmission).

Axes
- mc: drug_contact_multiplier
- rho: drug_contact_restore (contact restoration for treated high-strain)
- mr: drug_transmission_multiplier
- phi_transmission: virulence/transmission multiplier

Outputs
- Figures/drug_contact_restore_sweep/peak_infection_landscape.png

Run (from repo root)
    python Scripts/Drug_contact_restore_sweep_3D.py \
        --phi 1.0 1.2 1.5 1.8 2.2 \
        --mc 0.8 1.0 1.2 \
        --restore 0.0 0.5 0.8 1.0 \
        --mr 0.5 0.7 1.0 \
        --days 365 --steps 365

Notes
- Uses SEIRS_model_v7.
- Produces 3D surface landscapes over (mc, restore), faceted by phi_transmission and mr.
- Two rows per phi: high-strain peak and low-strain peak. Columns are mr values.
"""
...
def params_tuple_v7(phi_t: float, mc: float, mr: float, restore: float) -> Tuple[float, ...]:
...
def run_sim(phi_t: float, mc: float, mr: float, restore: float, days: int, steps: int) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
    t = np.linspace(0, days, steps)
    y0 = initial_conditions(normalize=True)
    sol = odeint(SEIRS_model_v7, y0, t, args=(params_tuple_v7(phi_t, mc, mr, restore),))
...
def peak_infection_landscape(df: pd.DataFrame, out_path: str) -> None:
    ...
    restore_vals = sorted(df["restore"].unique().tolist())
    mr_vals = sorted(df["mr"].unique().tolist())

    if len(mc_vals) < 2 or len(restore_vals) < 2:
        raise ValueError("Need at least 2 values each for --mc and --restore to plot a surface.")

    X, Y = np.meshgrid(restore_vals, mc_vals)  # columns=restore, rows=mc
    ...
    ncols = len(mr_vals)
    nrows = len(phi_vals) * 2
    fig = plt.figure(figsize=(4.2 * ncols, 3.6 * nrows))

    for i, phi_t in enumerate(phi_vals):
        for j, mr in enumerate(mr_vals):
            sub = df[
                np.isclose(df["phi_transmission"], float(phi_t))
                & np.isclose(df["mr"], float(mr))
            ]
            z_hi = (
                sub.pivot(index="mc", columns="restore", values="peak_I_high")
                .reindex(index=mc_vals, columns=restore_vals)
                .to_numpy(dtype=float)
            )
            z_lo = (
                sub.pivot(index="mc", columns="restore", values="peak_I_low")
                .reindex(index=mc_vals, columns=restore_vals)
                .to_numpy(dtype=float)
            )

            hi_index = i * 2 * ncols + j + 1
            lo_index = (i * 2 + 1) * ncols + j + 1
            ax_hi = fig.add_subplot(nrows, ncols, hi_index, projection="3d")
            ax_lo = fig.add_subplot(nrows, ncols, lo_index, projection="3d")
            ...
            ax_hi.set_title(f"phi={phi_t:g}, mr={mr:g}")
            ax_lo.set_title(f"phi={phi_t:g}, mr={mr:g}")

            ax_hi.set_xlabel("restore")
            ax_hi.set_ylabel("mc")
            ax_hi.set_zlabel("peak_I_high")

            ax_lo.set_xlabel("restore")
            ax_lo.set_ylabel("mc")
    ...
    fig.subplots_adjust(left=0.05, right=0.95, top=0.98, bottom=0.03, wspace=0.3, hspace=0.35)
    plt.savefig(out_path, dpi=600)
    plt.close(fig)


def main(argv: List[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Sweep drug contact multiplier vs contact restore (SEIRS v7)")
    ...
    ap.add_argument("--restore", nargs="+", type=float, default=None, help="drug_contact_restore values")
    ...
    restore_vals = args.restore if args.restore is not None else [0.0, 0.5, 0.8, 1.0]
    mr_vals = args.mr if args.mr is not None else [float(getattr(P, "drug_transmission_multiplier", 1.0))]

    phi_vals = sorted(set(float(x) for x in phi_vals))
    mc_vals = sorted(set(float(x) for x in mc_vals))
    restore_vals = sorted(set(float(x) for x in restore_vals))
    mr_vals = sorted(set(float(x) for x in mr_vals))

    fig_dir = os.path.join(ROOT_DIR, "Figures", "Model_v7_exploration", "drug_contact_restore_sweep")
        ...
        for mr in mr_vals:
            for mc in mc_vals:
                for restore in restore_vals:
                    t, sim = run_sim(phi_t, mc, mr, restore, args.days, args.steps)
                    m = metrics(t, sim)
                    ...
                    if args.debug and np.isclose(phi_t, 1.0) and np.isclose(mc, 1.0) and np.isclose(restore, 1.0):
                        print(f"\nResults for phi={phi_t}, mc={mc}, restore={restore}, mr={mr}:")
    ...
    df = pd.DataFrame(rows).sort_values(["phi_transmission", "mr", "mc", "restore"])\
