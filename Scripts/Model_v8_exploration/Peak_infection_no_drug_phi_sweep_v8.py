#!/usr/bin/env python3
"""Sweep virulence in SEIRS v8 and plot no-drug peak infection by strain.

Outputs:
  Tables/peak_infection_no_drug_phi_sweep_v8.csv
  Figures/Model_v8_exploration/peak_infection_no_drug_phi_sweep_v8.png
  Figures/Model_v8_exploration/peak_infection_no_drug_phi_sweep_v8.svg

Run from repo root:
  python Scripts/Model_v8_exploration/Peak_infection_no_drug_phi_sweep_v8.py \
...
def params_tuple_v8(phi_t: float) -> Tuple[float, ...]:
    return (
...
def run_sim(phi_t: float, days: int, steps: int) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
    ...
    sol = odeint(SEIRS_model_v8, initial_conditions(normalize=True), t, args=(params_tuple_v8(phi_t),))
...
def plot_results(results: List[Tuple[float, np.ndarray, Dict[str, np.ndarray]]], summary: pd.DataFrame, out_base: str) -> None:
    ...
    ax_ts.set_title("SEIRS v8: No-drug infectious dynamics across virulence")
    ...
    ax_peak.set_title("SEIRS v8: Peak infection vs virulence (no drug)")
...
def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="No-drug virulence sweep for SEIRS v8")
    ...
    print("=" * 72)
    print("SEIRS v8: No-drug virulence sweep")
    print("=" * 72)
    ...
    table_path = os.path.join(ROOT_DIR, "Tables", "peak_infection_no_drug_phi_sweep_v8.csv")
    figure_base = os.path.join(ROOT_DIR, "Figures", "Model_v8_exploration", "peak_infection_no_drug_phi_sweep_v8")
