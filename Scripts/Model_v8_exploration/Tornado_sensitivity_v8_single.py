...
from Models.SEIRS_Models import SEIRS_model_v8_singlestrain
...
# Parameter keys actively perturbed in v8 single-strain tornado analysis
# Keep phi_t, kappa_base, and kappa_scale fixed at baseline values.
PARAM_KEYS = [
    "contact_rate",
...
def initial_state_normalized():
    # v8 single-strain state: [S, El, Indl, Idl, Rl]
    y0 = np.array(
        [
            float(P.S),
            float(P.El),
...
def build_param_tuple(d):
    # v8 single expects (9):
    # (c_low, r_low, phi_t, m_r_drug, kappa_base, kappa_scale, sigma, tau, theta)
    return (
...
def simulate_metric(param_dict, metric):
    ...
    sol = odeint(SEIRS_model_v8_singlestrain, y0, t, args=(build_param_tuple(param_dict),))
...
def create_tornado_plot(rows, metric_name):
    ...
    ax.set_title("Tornado sensitivity (SEIRS_model_v8_singlestrain)")
    ...
    fig_path = os.path.join(out_dir, f"Tornado_v8_single_{metric_name}.svg")
...
def save_csv_results(rows, metric_name):
    ...
    csv_path = os.path.join(csv_dir, f"Tornado_v8_single_{metric_name}.csv")
...
print("=" * 60)
print("TORNADO SENSITIVITY ANALYSIS - SEIRS Model v8 Single Strain")
print("=" * 60)
