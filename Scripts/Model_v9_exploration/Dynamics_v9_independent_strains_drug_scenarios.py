"""
Independent-strain dynamics for SEIRS v9 (two-strain model, no competition).

This script uses SEIRS_model_v9 but runs each strain independently by zeroing
...
def compute_reproduction_numbers(params_tuple):
    """Return strain-specific R0 and Reff for v9 parameterization."""
    (
        c_low_i,
        r_low_i,
        phi_t_i,
...
def run_independent_scenario(seed_strain, params_tuple):
    """Run one independent-strain simulation using SEIRS_model_v9."""
...
print("=== v9 Independent-Strain Dynamics (No Competition) ===")
results = {}
...
out_base = os.path.join(
    ...
    "../../Figures/Model_v9_exploration/v9_independent_strains_susceptible_dynamics",
)
